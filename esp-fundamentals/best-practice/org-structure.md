...
- If at all possible, use the org structure mode '''team''' instead of '''hierarchical''' mode of the organizational structure. Basically, '''team''' mode allows you to create and work with a single flat list of teams for working with task and request assignments, instead of trying to define a simple hierarchical structure, team mode provides a much simpler interface to manage a single, global list of teams for assignment and grouping. '''Team''' mode should be sufficient for almost all organizations.
