...
- __An Individual User__: Any full user (see [User Account Types](/esp-fundamentals/security/user-accounts#user-account-types) to read more about account types) can have tasks assigned to them. It should be noted that it's possible for a user to be a member of more than one Assignment Team and/or Assignment Role.  Therefore when assigning a task to an individual user, it may be required that you assign the task to the user, _in the context of_ a specific team or role.  Why you might need to do this is discussed in further details below [Assignment Context](/esp-fundamentals/core-capabilities/human-tasks#task-assignment-context)
