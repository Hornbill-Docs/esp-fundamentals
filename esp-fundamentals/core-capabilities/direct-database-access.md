...
> The database schema presented in the DDA database is controlled and maintained by Hornbill's product teams. As we develop/evolve our products, we will ensure that any customer reporting-relevant data is included in the DDA database. When accessing the database through the Direct Database Access service, you access it read-only, giving you no option to directly add or change data, or modify the database schema.
